...
- [X] T001 Create source directories `src/discovery`, `src/extraction`, `src/verification`, `src/diagnosis`, `src/storage`, and `tests/` per implementation plan
- [X] T002 Configure Python runtime dependencies in `requirements.txt` (`streamlit`, `groq`, `pydantic`, `httpx`, `beautifulsoup4`, `psycopg2-binary`, `pytest`)
- [X] T003 [P] Configure environment settings and secret loading in `src/config.py` loading `GROQ_API_KEY`, `GROQ_MODEL`, and search keys from `.env`
...
- [X] T004 Create Pydantic domain models in `src/storage/models.py` (`Prospect`, `EvidenceSource`, `Claim`, `StrategicGap`, `AuditEvent`) per `data-model.md`
- [X] T005 Implement dual SQLite/PostgreSQL persistence engine in `src/storage/db.py` with table creation, connection pooling, and schema initialization
- [X] T006 [P] Implement audit event logger in `src/storage/audit.py` writing immutable timestamped events to database
- [X] T007 [P] Implement async HTTP web fetcher and text tokenizer in `src/discovery/web_fetcher.py` with sanitization, content-hashing (SHA-256), and timeout handling
- [X] T008 Implement live OSINT search provider in `src/discovery/search_client.py` querying live search (Tavily/DuckDuckGo) with explicit `ERR_LIVE_SEARCH_FAILED` on failure (no offline mocks)
- [X] T009 [P] Implement deterministic domain tier classifier in `src/discovery/tier_classifier.py` mapping domains to Tiers 1 through 4
...
- [X] T010 [P] [US1] Unit test for URL validation and slug parsing in `tests/test_ingest.py`
- [X] T011 [P] [US1] Unit test for source tier classification in `tests/test_discovery.py`
- [X] T012 [P] [US1] Integration test for double-check primary verification and contradiction detection in `tests/test_verification.py`

### Implementation for User Story 1
- [X] T013 [P] [US1] Implement LinkedIn URL validator and canonical slug parser in `src/discovery/ingest.py`
- [X] T014 [US1] Implement atomic claim extractor and category classifier in `src/extraction/claim_auditor.py` using Groq Llama 3.3 70B structured outputs
- [X] T015 [P] [US1] Implement claim materiality scorer in `src/extraction/materiality.py` (High/Medium/Low)
- [X] T016 [US1] Implement Check 1 (Primary Source Grounding & Entailment >= 0.85) in `src/verification/double_checker.py`
- [X] T017 [US1] Implement Check 2 (Independent Corroboration & Consistency) and contradiction detector in `src/verification/contradiction.py`
- [X] T018 [US1] Implement 3-state classifier hub in `src/verification/classifier_hub.py` assigning `VERIFIED`, `PARTIALLY_VERIFIED`, or `UNVERIFIED`
...
- [X] T019 [P] [US2] Unit test for refusal codes and explanation generation in `tests/test_refusal.py`

### Implementation for User Story 2
- [X] T020 [US2] Implement refusal taxonomy and exclusion processor in `src/verification/refusal_engine.py` supporting codes `REF-01` to `REF-05`
- [X] T021 [US2] Implement refusal ledger formatter and audit serialization in `src/verification/refusal_engine.py`
...
- [X] T022 [P] [US3] Unit test for three-gap synthesis and dimension ranking in `tests/test_gap_synthesizer.py`

### Implementation for User Story 3
- [X] T023 [US3] Implement 3-gap presence analyzer in `src/diagnosis/gap_synthesizer.py` using Groq Llama 3.3 70B
- [X] T024 [US3] Implement executive strategic recommendation generator in `src/diagnosis/gap_synthesizer.py`
...
- [X] T025 [US4] Implement interactive Human Gate staging table in `src/ui/human_gate.py` displaying claims, citation chips, and contradiction alerts
- [X] T026 [US4] Implement manual override action handler with mandatory rationale comment input in `src/ui/human_gate.py`
- [X] T027 [US4] Implement advisor sign-off authorization lock (`Approve & Compile Diagnostic`) in `src/ui/human_gate.py`
...
- [X] T028 [US5] Implement One-Page Diagnostic renderer in `src/diagnosis/diagnostic_renderer.py`
- [X] T029 [US5] Implement Streamlit executive diagnostic presentation card view in `src/ui/diagnostic_view.py`
- [X] T030 [US5] Implement export routines (Markdown and JSON audit trail) in `src/diagnosis/export.py`
...
- [X] T031 Implement end-to-end pipeline coordinator in `src/orchestrator.py` chaining Ingest ➔ Discovery ➔ Extraction ➔ Verification ➔ Refusal ➔ Gaps ➔ Human Gate ➔ Diagnostic
- [X] T032 Build primary Streamlit application in `src/app.py` tying together input URL form, live progress indicators, Human Gate, and One-Page Diagnostic
- [X] T033 Implement end-to-end candidate golden test in `tests/test_e2e.py` testing Ronaldo Mouchawar's public profile
- [X] T034 [P] Update documentation and verify all tests pass with `pytest tests/ -v`
